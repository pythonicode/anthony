...
function groundfindp(p, facts, rules) {
  ...
  if (symbolp(p)) {
    return groundfindatom(p, facts, rules);
  }
  if (p[0] === "same") {
    return equalp(p[1], p[2]);
  }
  if (p[0] === "distinct") {
    return !equalp(p[1], p[2]);
  }
  if (p[0] === "not") {
    return !groundfindp(p[1], facts, rules);
  }
  if (p[0] === "and") {
    return groundfindand(p, facts, rules);
  }
  if (groundfindbackground(p, facts, rules)) {
    return true;
  }
...
function groundcompute(rel, facts, rules) {
  ...
  for (var i = 0; i < data.length; i++) {
    if (symbolp(data[i])) {
      if (equalp(data[i], rel)) {
        answers.push(rel);
      }
    } else if (data[i][0] !== "rule") {
      if (equalp(operator(data[i]), rel)) {
        answers.push(data[i]);
      }
    } else {
      if (
        equalp(operator(data[i]), rel) &&
...
function groundfindand(p, facts, rules) {
  for (var i = 1; i < p.length; i++) {
    if (!groundfindp(p[i], facts, rules)) {
      return false;
    }
  }
  return true;
...
function groundfindrs(p, facts, rules) {
  ...
  for (var i = 0; i < data.length; i++) {
    if (symbolp(data[i])) {
      ...
      if (equalp(data[i][1], p) && groundfindsubs(data[i], facts, rules)) {
        return true;
      }
    }
  }
...
function groundfindsubs(rule, facts, rules) {
  for (var j = 2; j < rule.length; j++) {
    if (!groundfindp(rule[j], facts, rules)) {
      return false;
    }
  }
  return true;
...
function grounditem(rel, facts, rules) {
  ...
  for (var i = 0; i < data.length; i++) {
    if (symbolp(data[i])) {
      ...
      var head = data[i][1];
      if (operator(head) === rel && groundfindsubs(data[i], facts, rules)) {
        return head[1];
      }
    }
  }
...
function groundexpanddepth(seed, facts, rules, depth) {
  ...
  if (seed[0] === "and") {
    return groundexpanddepthand(seed, facts, rules, depth);
  }
  if (seed[0] === "transition") {
    return groundexpanddepthtransition(seed, facts, rules, depth);
  }
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (equalp(data[i][1], seed)) {
      ...
      updates = updates.concat(
        groundexpanddepth(rule, facts, rules, depth + 1)
      );
    }
  }
  if (flag) {
