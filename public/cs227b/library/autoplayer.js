...
async function doreceive() {
  ...
  for (var i = 0; i < messages.length; i++) {
    ...
    var response = await ggpeval(message);
    if (response) {
...
async function ggpeval(msg) {
  ...
  if (msg[0] === "start") {
    return await start(msg[2], msg[3], msg[4], msg[5]);
  }
  if (msg[0] === "play") {
    return await play(msg[2]);
  }
  if (msg[0] === "stop") {
    return stop(msg[2]);
  }
...
function play(role) {
  if (strategy === "legal") {
    return playlegal(role);
  }
  if (strategy === "random") {
    return playrandom(role);
  }
  if (strategy === "onestep") {
  ...
  if (strategy === "minimaxdepth") {
    return playminimaxdepth(role);
  }
  if (strategy === "minimaxid") {
...
function send(sender, receiver, msg) {
  var url =
    "http://127.0.0.1/worksheets/homepage/putmessage.php?sender=" +
    sender +
    "&receiver=" +
    receiver;
  return rpc(url, msg);
...
function send(sender, receiver, msg) {
  var url =
    "http://minimal.stanford.edu/worksheets/homepage/putmessage.php?sender=" +
    sender +
    "&receiver=" +
    receiver;
  return rpc(url, msg);
...
function receive(receiver) {
  return rpc(
    "http://127.0.0.1/worksheets/homepage/getmessages.php?receiver=" + receiver,
    ""
  );
}
...
function receive(receiver) {
  return rpc(
    "http://minimal.stanford.edu/worksheets/homepage/getmessages.php?receiver=" +
      receiver,
    ""
  );
}
