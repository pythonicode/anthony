...
function groundrules(library, deadline) {
  var facts = groundfacts(library, deadline);
...
function groundrule(rule, facts, rules) {
  ...
  if (rule[0] === "rule") {
    return groundviewrule(rule, facts, rules);
  }
  if (rule[0] === "handler") {
    return groundhandler(rule, facts, rules);
  }
  rules[rules.length] = rule;
  return rules;
}
...
function groundfacts(library, deadline) {
...
function compview(r, facts, library) {
  ...
  for (var i = 0; i < data.length; i++) {
    if (operator(data[i]) === r) {
      comprule(data[i], facts);
    }
  }
  return true;
}
...
function compsubgoals(n, rule, al, facts) {
  if (n >= rule.length) {
    compsave(plug(rulehead(rule), al), facts);
    return true;
  }
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (bl) {
      compsubgoals(n + 1, rule, bl, facts);
    }
  }
  return true;
}
...
function compsave(fact, facts) {
  ...
  if (find(fact, indexees(rel, facts))) {
    return fact;
  }
  facts.push(fact);
...
function getdependencies(rule, ds) {
  if (symbolp(rule)) {
    return setrelation(rule, ds);
  }
  var rel = operator(rule);
  if (rule[0] !== "rule") {
    return setrelation(rel, ds);
  }
  for (var j = 2; j < rule.length; j++) {
    setdepends(rel, operator(rule[j]), ds);
  }
  return ds;
}
...
function findallexp(x, p, pl, al, results, facts) {
  ...
  if (p[0] === "distinct") {
    return findalldistinct(x, p, pl, al, results, facts);
  }
  if (p[0] === "not") {
    return findallexp(x, p[1], pl, al, results, facts);
  }
  if (p[0] === "and") {
    return findalland(x, p, pl, al, results, facts);
  }
  return findallbase(x, p, pl, al, results, facts);
}
...
function findallbase(x, p, pl, al, results, facts) {
  for (var i = 0; i < facts.length; i++) {
    ...
    if (bl) {
      results = findallexit(x, pl, bl, results, facts);
    }
  }
  return results;
}
