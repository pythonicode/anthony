...
function compchangers(p, results, rules) {
  ...
  if (p[0] === "not") {
    return compchangersdb(p[1], results, rules);
  }
  if (p[0] === "and") {
...
function mentions(item, factoid) {
  ...
  if (item[0] === "not") {
    return equalp(item[1], factoid);
  }
  if (item[0] === "and") {
    for (var i = 1; i < item.length; i++) {
      if (mentions(item[i], factoid)) {
        return true;
      }
    }
    return false;
  ...
  if (item[0] === "handler") {
    if (equalp(item[1], factoid)) {
      return true;
    }
    if (mentions(item[2], factoid)) {
      return true;
    }
  }
  if (item[0] === "transition") {
    if (mentions(item[1], factoid)) {
      return true;
    }
    if (mentions(item[2], factoid)) {
      return true;
